# BUILD-LOG — claude-for-legal--claude-liam-case-brief

## 2026-08-29 — review cut, DONE

Redo-mode build (`mode: "redo"`) of
`anthropics/claude-for-legal/youtube/claude-liam-case-brief/beat_sheet.json`
— a 7-beat Teardown skill-explainer (`claude-liam` / @NikBearBrown) about the
legal-clinic skill `case-brief`.
...
`claude-for-legal--claude-liam-build-guide` sibling redo. The SKILL.md
case-brief was meant to describe
(`/Users/bear/Documents/CoWork/bear-textbooks/books/anthropics/claude-for-legal/law-student/skills/case-brief/SKILL.md`)
...
`law-student/` directory). This is NOT a "genuine blocker that halts the
...
source: any specific claim about what case-brief's particular legal-clinic
procedure does. This redo treats "case-brief" literally and generically —
the named example of a skill-shaped folder, using only the well-known,
generic shape of a legal case brief (facts, issue, rule, conclusion) as
...
"a case-brief skill" sounds like a legal upgrade), B02 (wrong guess broken
with a falsifying case — delete the skill's folder, no case law is
...
a clean brief proves nothing about understanding; a messy one proves
...
shape to the `claude-for-legal--claude-liam-build-guide` sibling redo,
which hit the same source-fidelity gap.

**B00 WRITER LAW:** wrong guess — a newcomer assumes "Claude has a
case-brief skill" means the model itself learned case law. Typed text:
"Claude learned case law / from the case-brief skill. / What is a skill?",
trigger "learned" → replacement "was given", ending on the real question.
Audio 10.47s (Remotion extended to 10.5s) — clears the ≥8s WRITER LAW floor
with margin; verified on a late frame (t=9.5s) that the correction resolves
to "was given" and the full question types out to "What is a skill?" before
the beat ends.

**Body beats (B01-B07):** all Manim GRAPHIC scenes, reusing the generic
"chip row" / "chip stack" renderer pattern from the
`claude-for-legal--claude-liam-build-guide` sibling redo verbatim, adapted
in this reel's own `scenes.py` with case-brief-specific chip labels and
narration. Anchor pair: B03 plants `case-brief/` + `SKILL.md` as two plain
...
**GATE T (type_check.py): PASS on first run** (0 FAILs across all 11
beats) — the chip-row/chip-stack renderer's known kerning/bbox-overlap
fixes (upright serif captions, MUTE-not-strikethrough for rejected chips,
`render_chip_stack`'s one-phrase-per-row layout for B07) were reused
directly from the build-guide sibling, so no new type-check defect
surfaced.

**Compile:** `compile.py` forced a clean 3840×2160 master directly (no
declared slates — all 11 beats real media).
`claude-for-legal--claude-liam-case-brief.mp4`, 111.0s. One non-blocking
...
correction frame confirmed at t=9.5s (mid-beat, well after the "was given"
correction resolves).
...
**Master vs. beat_sheet.json:** master mtime (11:47) is newer than
beat_sheet.json's last content edit (11:45) — beat_sheet.json was NOT
...
**Delivery:** `claude-for-legal--claude-liam-case-brief-4k.mp4` created — a
...
`claude-for-legal--claude-liam-case-brief.md` (YouTube description,
@HumanitariansAI, playlist "Claude Basics", direct code link, AI
disclosure). Ran `deliver.py --push` to stage
`DELIVERY/claude-for-legal--claude-liam-case-brief/` and commit text
artifacts to the humanitarians-youtube clone under
`claude-bear/claude-for-legal--claude-liam-case-brief/`.
...
claimed" section — nothing about the actual legal-clinic case-brief
